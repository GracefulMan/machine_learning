...
def sgd(train_x, train_y, learning_rate=0.05, iter_num = 10000 , show_loss = False):
    sample_nums = train_x.shape[0]
    features = train_x.shape[1]
    beta = np.random.normal((features, 1)).reshape((features, 1))
    for i in range(iter_num):
        for j in range(sample_nums):
            tmp = np.dot(train_x.T, np.dot(train_x, beta) - train_y)
            beta -= learning_rate * tmp
        if show_loss:
            print(mse(train_y, np.dot(train_x, beta)))
...
def sgd_with_mini_batch(train_x, train_y, learning_rate=0.0001,iter_num = 1000, batch_size=32):
    sample_nums = train_x.shape[0]
    features = train_x.shape[1]
    beta = np.random.normal((features, 1)).reshape((2, 1))
    for i in range(iter_num):
...
def sgd_with_mini_batch_diff_loss(train_x, train_y, learning_rate=1e-5, loss='mse', iter_num = 10000, batch_size =32,show_loss=False):
    sample_nums = train_x.shape[0]
    features = train_x.shape[1]
    beta = np.random.normal((features, 1)).reshape((2, 1))
    for i in range(iter_num):
...
if __name__ == '__main__':
    train_x, train_y, _ = generate_dataset(100, 2)
    ...
    beta5 = sgd_with_mini_batch_diff_loss(train_x, train_y, loss='mae', show_loss= True, iter_num=10000)
